...
pub async fn start_new_task(
    ...
    task_info: web::Json<NewTaskInfo>,
) -> impl Responder {
    let task_name = task_info.name.to_lowercase();
    let dupe_count = match sqlx::query!(
            "SELECT COUNT(*) FROM tasks WHERE name = $1 AND finished_at IS NULL;", 
            task_name,
        )
    ...
    let new_task_op = sqlx::query_as!(
            TaskModel,
            "INSERT INTO tasks (name, user_github_id, started_at) VALUES ($1, $2, $3) RETURNING *;",
            task_name,
